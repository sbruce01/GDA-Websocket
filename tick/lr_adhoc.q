...
/ .u.end:{0N!"Starting EOD at: ",string[.z.p];t:.lr.tables;t@:where `g=attr each t@\:`sym;.Q.dpft[hsym `$HDBDIR;x;`sym;.lr.tables];@[;`sym;`g#] each t;0N!"Finishing EOD at: ",string[.z.p];};
...
// change to hdb dir for write down

// call rep over each table group
...
requiredTables:(`trade;`order;`ethereum;`ohlcv;`vwap);  // Tables we want
![`.;();0b;requiredTables];  // Delete from top namespace

{.lr.tables:y;y set .storedTables[y];.u.rep[x;z];![`.;();0b;enlist[y]]}[LOGFILE;;saveDate] each (`trade;`order;`ethereum;`ohlcv;`vwap);
.Q.chk[hsym `$HDBDIR]